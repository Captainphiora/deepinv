...
MOTIONBLUR_REPO="/mnt/afs/L202500464/DiffPIR/motionblur"
MOTIONBLUR_COMMIT="578137887b2067c532bd4e422ff5afdcabfccebf"
MOTIONBLUR_SOURCE_SHA256="71c81f9249d88dfdabbdb6481f07181a32844179fb7affe1f62836a9348815fc"
...
RUN_ID="motion-deblur-$(date -u +%Y%m%dT%H%M%SZ)"
SETTING_IDS=(
  ffhq256_motion61_intensity05_sigma005_diffpir_quad20_v1
  ffhq256_motion61_intensity05_sigma005_diffpir_quad100_v1
  ffhq256_motion61_intensity05_sigma000_diffpir_quad20_v1
  ffhq256_motion61_intensity05_sigma000_diffpir_quad100_v1
)
...
test -d "${MOTIONBLUR_REPO}"
test -d "${IMAGES}"
test -f "${CHECKPOINT}"
test "$(git -c safe.directory="${MOTIONBLUR_REPO}" -C "${MOTIONBLUR_REPO}" rev-parse HEAD)" = "${MOTIONBLUR_COMMIT}"
test "$(sha256sum "${MOTIONBLUR_REPO}/motionblur.py" | cut -d ' ' -f 1)" = "${MOTIONBLUR_SOURCE_SHA256}"
cd "${ROOT}"
export PYTHONUNBUFFERED=1

echo "[1/5] Preparing four immutable five-image motion fixtures"
for setting_id in "${SETTING_IDS[@]}"; do
  ...
  if [[ ! -f "${ARTIFACT_ROOT}/fixtures/${setting_id}/manifest.json" ]]; then
    "${PYTHON}" reproduction/diffpir/prepare_inputs.py \
      ...
      --motionblur-repo "${MOTIONBLUR_REPO}" --limit 5 \
...
echo "Completed one task only: motion deblur"
echo "Run ID: ${RUN_ID}"
