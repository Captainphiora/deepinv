...
RUN_ID="gaussian-deblur-$(date -u +%Y%m%dT%H%M%SZ)"
SETTING_IDS=(
  ffhq256_gaussian61_std3_sigma005_diffpir_quad20_v1
  ffhq256_gaussian61_std3_sigma005_diffpir_quad100_v1
  ffhq256_gaussian61_std3_sigma000_diffpir_quad20_v1
  ffhq256_gaussian61_std3_sigma000_diffpir_quad100_v1
)
...
echo "[1/5] Preparing four immutable five-image fixtures"
for setting_id in "${SETTING_IDS[@]}"; do
  ...
  if [[ ! -f "${ARTIFACT_ROOT}/fixtures/${setting_id}/manifest.json" ]]; then
    "${PYTHON}" reproduction/diffpir/prepare_inputs.py \
      ...
      --limit 5 --artifact-root "${ARTIFACT_ROOT}"
  fi
done
...
echo "Completed one task only: Gaussian deblur"
echo "Run ID: ${RUN_ID}"
